const API_KEY = "e3d6e2964d394cf6b39a1e648f35f4a4";  
...
// ===== Category Card Click: Fetch Recipes by Category =====
document.querySelectorAll('.category-card').forEach(card => {
  card.addEventListener('click', function() {
    const category = card.getAttribute('data-category');
    let url = "";
    if (category === "italian" || category === "indian") {
      url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&cuisine=${encodeURIComponent(category)}&number=10&addRecipeInformation=true`;
    } else if (category === "high protein") {
      url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&minProtein=25&number=10&addRecipeInformation=true`;
    } else if (category === "drinks") {
      url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&type=drink&number=10&addRecipeInformation=true`;
    } else {
      url = `https://api.spoonacular.com/recipes/complexSearch?apiKey=${API_KEY}&includeIngredients=${encodeURIComponent(category)}&number=10&addRecipeInformation=true`;
    }
    document.getElementById('recipes').scrollIntoView({ behavior: 'smooth' });
    fetchCategoryRecipes(url);
  });
});

// Helper for category fetch
async function fetchCategoryRecipes(url) {
  recipesContainer.innerHTML = "<p class='loading'>Loading recipes...</p>";
  cutBtn.style.display = "none";
  try {
    const res = await fetch(url);
...
// Smooth scroll for nav-list links with header offset for Search
navLinks.forEach(link => {
